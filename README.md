...
**功能特点**：
- **课程搜索**：支持按课程号或课程名称进行搜索
- **多条件筛选**：支持按学院、课程类型（必修/选修/公选）、学分范围、上课时间筛选
- **课程列表**：表格/卡片形式展示，显示课程名称、授课教师、学分、上课时间地点、容量（已选/总数）、课程状态
- **选课状态**：清晰显示可选/已满/已选状态，按钮状态动态变化
- **选课管理**：支持选课/退课操作，操作结果实时反馈
- **选课统计**：显示已选课程数量、已修学分、时间冲突警告
- **实时更新**：选课/退课后，列表和统计信息自动刷新
- **数据同步**：选课成功后，推荐系统自动更新

**选课流程**：
1. 使用搜索框或筛选条件查找课程
2. 查看课程详细信息（学分、容量、时间等）
3. 点击"选课"按钮
4. 系统验证容量、时间冲突等条件
5. 选课成功后，列表和统计自动更新
6. 推荐系统自动更新推荐结果
...
| 接口路径 | 方法 | 说明 |
|---------|------|------|
| `/api/get_filtered_courses` | GET | 获取筛选后的课程列表（支持搜索和筛选） |
| `/api/get_selection_statistics` | GET | 获取选课统计信息（已选数/学分/冲突） |
| `/api/select_course` | POST | 选课操作 |
| `/api/drop_course` | POST | 退课操作 |
